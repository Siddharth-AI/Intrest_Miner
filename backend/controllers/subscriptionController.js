...
//         }
//         // Amount in paise
//         const amount = Math.round(plan.price * 86.09 * 100);
...
const createRazorpayOrder = async (req, res) => {
    try {
        ...
        // Amount in paise (assuming INR, adjust conversion factor if needed)
        const amount = Math.round(plan.price * 87 * 100); // Example conversion, adjust as per your currency and Razorpay's expectations
        ...
        console.log(paymentData, "backend payment data=>>>>>>>>>>>>>>>>>>>>")
        await insertRecord("payments", paymentData);
        return res.status(200).json({
            success: true,
            order,
