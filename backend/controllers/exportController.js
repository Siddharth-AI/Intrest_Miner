 ...
 * 🔥 NOW USES CACHED AI FROM FRONTEND PAYLOAD
 */
    ...
    for (const campaign of campaigns) {
      try {
        ...
        if (!insights || insights.length === 0) {
          ...
          // 🔥 CRITICAL FIX: Use exact field names from frontend
          campaignsForAnalysis.push({
            ...
            insights: [],
            hasData: false,
            // 🔥 Read AI fields exactly as frontend sends them
            aiverdict: campaign.aiverdict || 'No verdict',
            aianalysis: campaign.aianalysis || 'No analysis',
            airecommendations: campaign.airecommendations || 'No recommendations'
          });
          continue;
        }
...
`);
    ...
    for (let i = 0; i < campaignsForAnalysis.length; i++) {
      ...
      console.log(`\n📊 Campaign ${i + 1}: ${campaignData.campaignName}`);
      console.log(`  aiverdict: "${campaignData.aiverdict}"`);
      console.log(`  aianalysis: "${campaignData.aianalysis?.substring(0, 50)}..."`);

      // 🔥 FIXED: Use fields exactly as they come from frontend
      aiAnalysisResults.push({
        ...
        ai_verdict: campaignData.aiverdict || 'No verdict available',
        ai_analysis: campaignData.aianalysis || 'No analysis available',
        ai_recommendations: campaignData.airecommendations || 'No recommendations available'
      });

      // Add details sheet rows (if included)
      if (includeCampaignDetails && detailsSheet) {
        ...
        for (let j = 0; j < insights.length; j++) {
          ...
          ['impressions', 'clicks', 'reach', 'purchases', 'add_to_cart', 'initiate_checkout',
            'add_payment_info', 'view_content', 'landing_page_view', 'search', 'total_conversions'].forEach(col => {
              const cell = row.getCell(col);
              cell.numFmt = '#,##0';
              cell.alignment = { horizontal: 'right', vertical: 'middle' };
            });
    ...
    const cleanAccountName = accountName.replace(/[^a-zA-Z0-9_-]/g, '_');
